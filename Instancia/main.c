...
int main(int argc, char** argv) {
	...
	while (escucha) {
		...
		case OP_STORE:
			...
			log_trace(logger,"Se recibio la clave (%s) del coordinador",clave);
			if (resultado < 0) {
		...
		case RELEVANTAR_INSTANCIA:
			...
			log_trace(logger,"cantidad de claves a relevantar (%d)",cantidadClaves);
			for (int i = 0; i < cantidadClaves; i++) {
				char* clave_recibida = NULL;
				try_recibirPaqueteVariable(socketCoordinador,(void**) &clave_recibida);
				...
				if ((arch = fopen(path, "r")) == NULL) {
					log_info(logger," la instancia no encuentra el archivo de la clave %s",clave_recibida);
					free(path);
					free(clave_recibida);
					continue;
				}
...
void responder_solicitud_clave(int sockfd){
	...
	log_trace(logger,"se encontro la entrada numero(%d) en la tabla de entradas con tamanio (%d)", entrada->numero,entrada->tamanio);
	char* valor = buscarEnALmacenamiento(entrada->numero,entrada->tamanio);
