...
int hacer_set(char* clave, char* valor) {
	...
	log_trace(logger,"Se procede a ver si esa clave(%s) ya tenia un valor asociado",clave);
	...
	log_trace(logger,"La clave %s no tenia un valor asociado, se procede a encontrarle un lugar",clave);
...
int STORE(char* clave) {
	...
	log_trace(logger,"Se procede a ver si la clave (%s) esta en tabla de entradas",clave);
	tablaE* cv = buscarEntrada(clave);
	if (cv == NULL) {
		return -1;
	}
	log_trace(logger,"Se busca el valor de la clave (%s) en el almacenamiento",clave);
...
void almacenarEnDumper(char* data, char* clave, unsigned int tamanio) {
	...
	if (dictionary_has_key(dumper->fd, clave)) {
		log_trace(logger,"se encontro la clave (%s) en el diccionario de file descriptors", clave);
